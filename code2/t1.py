...
df_total_com_prd=spark.sql("select product_name, round(sum(total_commission),2) as total_commission from (select ord_itm.product_id, any_value(product_name) as product_name, sum(ord_itm.commission_amount) as total_commission from ord_itm join prd on ord_itm.product_id=prd.product_id group by ord_itm.product_id) group by product_name")

#order per city
df_total_ord_city=spark.sql("select city, count(*) as total_orders from(select ord_itm.customer_id,any_value(city) as city, count(DISTINCT ord_itm.order_id) as total_orders from ord_itm join cust on ord_itm.customer_id=cust.customer_id where ord_itm.item_status='Completed' group by ord_itm.customer_id) group by city")

# most return product
df_cus_return=spark.sql("select * from(select prd.product_id,any_value(product_name) as product_name,round(SUM(CASE WHEN ord_itm.item_status = 'Refunded' THEN ord_itm.quantity ELSE 0 END)  / SUM(ord_itm.quantity) *100,2)AS return_rate,round(sum(ord_itm.commission_amount),2) as commission_lost from prd join ord_itm on ord_itm.product_id=prd.product_id group by prd.product_id)where return_rate>50")

#below avd saller
df_worst_seller=spark.sql("select shope_name,total_sales from(select any_value(shop_name) as shope_name,round(sum(ord_itm.line_total),2) as total_sales,avg(sum(ord_itm.line_total)) over() as avg_sales from ord_itm join prd on ord_itm.product_id=prd.product_id join seller on prd.seller_id=seller.seller_id group by prd.seller_id )where total_sales<avg_sales")
...
df_higest_sales_day.write \
  ...
  .option("dbtable", "rutvik.test_write") \
  .option("user", "admin") \
